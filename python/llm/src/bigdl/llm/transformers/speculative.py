...
def update_attention_mask(attention_mask, len=1):
    return torch.cat(
        [attention_mask, attention_mask.new_ones((attention_mask.shape[0], len))], dim=-1)
...
def speculative_generate(self,
    ...
    while True:
        ...
        if step == 0:
            ...
            attention_mask = update_attention_mask(attention_mask, 1)
            step += 1
            ...
            for step_draft in range(max_step_draft):
                if self.config.model_type == "chatglm":
                    ...
                    draft_attention_mask = update_attention_mask(attention_mask, step_draft + step - 1)
                    draft_output = draft_model(input_ids=draft_current_input_ids,
                                               past_key_values=draft_past_key_values,
                ...
                draft_attention_mask = update_attention_mask(draft_attention_mask, 1)
                # check if draft prob is less then th_stop_draft
                # Draft number + step >= max output token number
                if min(draft_output_probs).item() < th_stop_draft or step + step_draft + 2 >= max_new_tokens:
                    break
            if self.device.type == 'xpu':
                torch.xpu.synchronize()
            toc = time.time()
            ...
            if self.config.model_type == "chatglm":
                ...
                cur_attention_mask = update_attention_mask(attention_mask, drafted_input_ids.size(1) + step - 2)
                output = self(input_ids=drafted_input_ids,
                              past_key_values=past_key_values,
            ...
            max_matched = min(((output_ids[:, :-1] != drafted_input_ids[:, 1:]).cumsum(-1) == 0).sum(-1)).item() + 1
